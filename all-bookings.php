...
<body>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        .then(data => {
            data.forEach(appointment => {
                let tableBody;
                if (appointment.order_status === 'Completed' || appointment.order_status === 'Finished') {
    ...
    function viewDetails(appointmentId) {
        ...
        .then(data => {
            ...
            const orderStatusOptions = ['Completed', 'Finished', 'Canceled'].map(status => 
                ...
                <div class="modal-section" style="text-align: center;">
                        ...
                        <div class="photo-gallery before">
                            ...
                            <div class="image-container">
                                ${data.beforePhotos.length > 0 ? data.beforePhotos.map(photo => `<img src="${photo}" alt="Before Photo" class="clickable-image">`).join('') : '<p>No photos available.</p>'}
                            </div>
                        </div>
                        <br>
                        ...
                        <div class="photo-gallery final">
                            ...
                            <div class="image-container">
                                ${data.finalPhotos.length > 0 ? data.finalPhotos.map(photo => `<img src="${photo}" alt="Final Photo" class="clickable-image">`).join('') : '<p>No photos available.</p>'}
                            </div>
                        </div>
                        <br>
                        <button onclick="sendReceipt(${data.appointment_id})" class="jotform-submit">Send Receipt</button>
                </div>
            `;
    ...
    function sendReceipt(appointmentId) {
        fetch('assets/php/receiptDetails.php', {
            method: 'POST',
            headers: {
        ...
        .then(response => response.json())
        .then(data => {
            // Assuming data contains all the necessary booking details
            const url = `https://workyarder.com/assets/php/customer-email.html?data=${encodeURIComponent(JSON.stringify(data))}&appointmentId=${appointmentId}`;
